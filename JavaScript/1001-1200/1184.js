...
var distanceBetweenBusStops = function (distance, start, destination) {
  ...
  // flip values
  start > destination ? [start, destination] = [destination, start] : null

  let sum = 0
  let clock = 0
  for (let i = 0; i < len; i++) {
    const val = distance[i]
    if (i >= start && i < destination) clock += val
    sum += val
  }

  // let counterClock = sum - clock;
  console.log(sum, clock, counterClock)
  return Math.min(clock, counterClock)
}

test(
  distanceBetweenBusStops([1, 2, 3, 4], 0, 1), // 1
  distanceBetweenBusStops([1, 2, 3, 4], 0, 2), // 3
