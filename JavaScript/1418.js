...
function displayTable (orders) {
  let foodItem = new Set()
  let tableNum = new Set()
  const map = {}

  orders.forEach(order => {
    const [table, dish] = [order[1], order[2]]
    foodItem.add(dish)
    tableNum.add(table)
    ...
    if (!(table in map)) {
      ...
      if (!map[table][dish]) {
        ...
        map[table][dish] = map[table][dish] + 1
      }
    }
  })

  console.log(map)
  foodItem = new Array(...foodItem).sort()
  tableNum = new Array(...tableNum).sort((a, b) => a - b)

  const menu = ['Table']
  menu.push(...foodItem)

  const body = [menu]
  tableNum.forEach(table => {
    const count = [table]
    for (let i = 1; i < menu.length; i++) {
      if (map[table][menu[i]]) {
        count.push(map[table][menu[i]] + '')
      } else {
        count.push(0 + '')
      }
    }
