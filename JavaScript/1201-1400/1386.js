...
function maxNumberOfFamilies (n, reservedSeats) {
  ...
  // populates map
  reservedSeats.forEach(([row, seat]) => !cache.has(row) ? cache.set(row, [seat]) : cache.get(row).push(seat))
  ...
  for (const [_, val] of cache) {
    ...
    /*
      if theres a seat taken in all 3 regions, its not possible to have any group of 4 adjacent => count -=2
      if any of the three regions are taken but not all 3, 1 group of 4 can fit => count--
      otherwise 2 groups can fit => do nothing
    */
    if (l && r && m) count -= 2
    else if (l || r || m) count--
    // console.log(_, val, count, l, m, r)
  }
...
test(
  maxNumberOfFamilies(3, [[1, 2], [1, 3], [1, 8], [2, 6], [3, 1], [3, 10]]), // 4
  maxNumberOfFamilies(2, [[2, 1], [1, 8], [2, 6]]), // 2
  maxNumberOfFamilies(4, [[4, 3], [1, 4], [4, 6], [1, 7]]) // 4
)
